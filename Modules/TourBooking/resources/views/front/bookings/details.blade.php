            ...
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{ route('front.tourbooking.my-bookings.invoice', $booking->booking_code) }}" class="btn btn-sm btn-light me-2">
                                <i class="bi bi-file-earmark-text"></i> {{ __('translate.View Invoice') }}
                            </a>
                            <a href="{{ route('front.tourbooking.my-bookings.download-invoice', $booking->booking_code) }}" class="btn btn-sm btn-light">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <p><strong>{{ __('translate.Booking Status') }}:</strong> 
                                <span class="badge bg-{{ $booking->booking_status == 'confirmed' ? 'success' : ($booking->booking_status == 'pending' ? 'warning' : ($booking->booking_status == 'cancelled' ? 'danger' : 'info')) }}">
                            ...
                            <p><strong>{{ __('translate.Total Amount') }}:</strong> {{ currencyConverter($booking->total) }}</p>
                            <p><strong>{{ __('translate.Paid Amount') }}:</strong> {{ currencyConverter($booking->paid_amount) }}</p>
                            @if($booking->due_amount > 0)
                            <p><strong>{{ __('translate.Due Amount') }}:</strong> {{ currencyConverter($booking->due_amount) }}</p>
                            @endif
                        </div>
                        <div class="col-md-6">
                            <h6 class="text-muted">{{ __('translate.Service Information') }}</h6>
                            <p><strong>{{ __('translate.Service') }}:</strong> {{ $booking->service->title }}</p>
                            <p><strong>{{ __('translate.Type') }}:</strong> {{ $booking->service->serviceType->name ?? 'N/A' }}</p>
                            <p><strong>{{ __('translate.Check-in Date') }}:</strong> {{ date('d M Y', strtotime($booking->check_in_date)) }}</p>
                            @if($booking->check_out_date)
                            <p><strong>{{ __('translate.Check-out Date') }}:</strong> {{ date('d M Y', strtotime($booking->check_out_date)) }}</p>
                            @endif
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#leaveReviewModal">
...
<div class="modal fade" id="cancelBookingModal" tabindex="-1" aria-labelledby="cancelBookingModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('front.tourbooking.my-bookings.cancel', $booking->booking_code) }}" method="POST">
                @csrf
...
<div class="modal fade" id="leaveReviewModal" tabindex="-1" aria-labelledby="leaveReviewModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('front.tourbooking.my-bookings.review', $booking->booking_code) }}" method="POST">
                @csrf
                <div class="modal-header">
